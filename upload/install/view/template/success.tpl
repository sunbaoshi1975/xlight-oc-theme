<?php echo $header; ?>
<div class="container">
  <header>
    <div class="row">
      <div class="col-sm-12"><img src="view/image/logo.png" alt="OpenCart" title="OpenCart" /></div>
    </div>
  </header>
  <h1>Install Octheme - Complete!</h1>
  <div class="row">
    <div class="col-sm-9">
      <div class="alert alert-danger">Don't forget to delete your installation directory!</div>
      <p>Congratulations! You have successfully install Octheme.</p>
      <br/>
      <p><span style="color: #FF0000">Please - Goto Admin and Edit the main System Settings. Update all fields and click save</span></p>
      <p><span style="color: #FF6699">Goto Admin -> Extensions -> Modifications - Check All boxes - Click Refresh button.</span></p>

      <div class="row">
        <div class="col-lg-6"><a href="../admin/"><img src="view/image/screenshot_2.png" alt="" class="img-thumbnail" /></a><br />
          <p class="text-center"><a href="../admin/">Login to your Administration</a></p>
        </div>
      </div>
    </div>
    <div class="col-sm-3">
      <ul class="list-group">
        <li class="list-group-item">Install</li>
        <li class="list-group-item"><b>Finished</b></li>
      </ul>
    </div>
  </div>
</div>
<?php echo $footer; ?>